...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _shaders = [cml.device newLibraryWithFile: @"mpr.metallib" error:&error];
        if (!_shaders)
        {
        ...
        id<MTLFunction> fragmentProgramA = [_shaders newFunctionWithName:@"frag_a"];
        if (!fragmentProgramA)
        {
            printf("ERROR: Couldn't load fragment function from default library.");
            return nil;
        ...
        pipelineStateDesc.fragmentFunction = fragmentProgramA;
        ...
        _framePipelineState = [cml.device newRenderPipelineStateWithDescriptor:pipelineStateDesc
                                                                    error:&error];
...
- (void)update {
    ...
    if (!cdl) {
        ...
        id<MTLTexture> mainPassFramebuffer = [cdl.layer.device newTextureWithDescriptor:texDesc];
        ...
        colorAttachment.texture = cdl.texture;
        [encoder setRenderPipelineState:_framePipelineState];
        ...
        [encoder setVertexBuffer:_uniformBuffer
                          offset:0 atIndex:FrameUniformBuffer];
        [encoder setFragmentTexture: mainPassFramebuffer atIndex: 0];
        // [encoder setViewport:{0, 0, 800, 600, 0, 1}];
    //    [encoder drawPrimitives:MTLPrimitiveTypeTriangle vertexStart:0 vertexCount:2*3];
        [encoder endEncoding];
