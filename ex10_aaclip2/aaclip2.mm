...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _shaders = [cml.device newLibraryWithFile: @"aaclip2sh.metallib" error:&error];
        if (!_shaders)
        {
        ...
        id<MTLFunction> stencil2tex = [_shaders newFunctionWithName:@"stencil2tex"];
        _computePipelineState = [cml.device newComputePipelineStateWithFunction:stencil2tex
                                                                       error:&error];
    ...
    if (self.sizeUpdated)
    {
        ...
        for (int i = width/2 - 10; i < width/2 + 10; i++) {
            for (int j = height/2 - 10; j < height/2 + 10; j++) {
                ((char*)buff.contents)[j*width + i] = 0xff;
            }
        }
        ...
        memset(aaBuff.contents, 0, aaWidth * height);


//        for (int i = aaWidth/2 - 100*4; i < aaWidth/2+100*4; i++) {
//            for (int j = height/2-100; j < height/2+100; j++) {
//                ((char*)aaBuff.contents)[j*aaWidth + i] = 0xff;
//            }
//        }


        id<MTLCommandBuffer> commandBuf;

//        commandBuf = [_commandQueue commandBuffer];
//        commandBuf = [_commandQueue commandBuffer];
//        id<MTLBlitCommandEncoder> blitEncoder = [commandBuf blitCommandEncoder];
//        [blitEncoder fillBuffer:buff range:NSMakeRange(0, width*height) value:0xff];
//        [blitEncoder endEncoding];
//        [commandBuf commit];
//        [commandBuf waitUntilCompleted];

        commandBuf = [_commandQueue commandBuffer];
        id<MTLComputeCommandEncoder> computeEncoder = [commandBuf computeCommandEncoder];
        [computeEncoder setComputePipelineState:_computePipelineState];

        [computeEncoder setBuffer:buff offset:0 atIndex:0];
        [computeEncoder setBuffer:aaBuff offset:0 atIndex:1];
        NSUInteger threadGroupSize = _computePipelineState.maxTotalThreadsPerThreadgroup;
        if (threadGroupSize > buff.length)
        {
            threadGroupSize = buff.length;
        }
        [computeEncoder dispatchThreadgroups:MTLSizeMake(buff.length, 1, 1)
...
//        id <MTLRenderCommandEncoder> encoder = nil;
        ...
        [encoder3 setFragmentTexture: _aaStencilData atIndex: 1];
