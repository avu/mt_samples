...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _shaders = [cml.device newLibraryWithFile: @"aarect2.metallib" error:&error];
        if (!_shaders)
        {
        ...
        _uniformBuffer3 = [cml.device newBufferWithLength:sizeof(FrameUniforms)
                                                  options:MTLResourceCPUCacheModeWriteCombined];
        _uniformBuffer4 = [cml.device newBufferWithLength:sizeof(FrameUniforms)
                                                  options:MTLResourceCPUCacheModeWriteCombined];
        ...
        pipelineStateDesc.alphaToCoverageEnabled = YES;
        pipelineStateDesc.alphaToOneEnabled = YES;
        ...
        _pipelineState1 = [cml.device newRenderPipelineStateWithDescriptor:pipelineStateDesc
                                                                     error:&error];
        pipelineStateDesc.rasterSampleCount      = 8;
        pipelineStateDesc.vertexFunction   = vertexProgram;
        pipelineStateDesc.fragmentFunction = fragmentProgram;
        pipelineStateDesc.vertexDescriptor = vertDesc;
        pipelineStateDesc.colorAttachments[0].sourceRGBBlendFactor = MTLBlendFactorSourceAlpha;
        ...
        pipelineStateDesc.vertexDescriptor = vertDesc;
        pipelineStateDesc.colorAttachments[0].sourceRGBBlendFactor = MTLBlendFactorOne;
        pipelineStateDesc.colorAttachments[0].sourceAlphaBlendFactor = MTLBlendFactorOne;
        pipelineStateDesc.colorAttachments[0].destinationRGBBlendFactor = MTLBlendFactorOneMinusSourceAlpha;
        pipelineStateDesc.colorAttachments[0].destinationAlphaBlendFactor = MTLBlendFactorOneMinusSourceAlpha;
        ...
        // Create vertices.
        Vertex verts[] = {
                Vertex{{-0.55f, -0.5f, 0}, {255, 255, 0, 255}},
                Vertex{{0.55, -0.5f, 0}, {255, 255, 0, 255}},
                Vertex{{-0.55, 0.5f, 0}, {255, 255, 0, 255}},
                Vertex{{0.55, 0.5f, 0}, {255, 255, 0, 255}}
        };
        Vertex verts1[] = {
                Vertex{{-0.55f, -0.5f, 0}, {255, 0, 0, 255}},
                Vertex{{0.55, -0.5f, 0}, {255, 0, 0, 255}},
                Vertex{{-0.55, 0.5f, 0}, {255, 0, 0, 255}},
                Vertex{{0.55, 0.5f, 0}, {255, 0, 0, 255}}
        };
    ...
    if (!cdl) {
       ...
       // colorAttachment.resolveTexture = cdl.texture;
        ...
        simd::float4x4 rot(simd::float4{(float)cos(0.3),-(float)sin(0.3), 0, 0},
                           ...
                           simd::float4{       0,        0, 1, 0},
                           simd::float4{       0,        0, 0, 1});
        simd::float4x4 rot2(simd::float4{(float)cos(0.6),-(float)sin(0.6), 0, 0},
                           simd::float4{(float)sin(0.6), (float)cos(0.6), 0, 0},
                           simd::float4{       0,        0, 1, 0},
                           simd::float4{       0,        0, 0, 1});
        ...
        uniforms3->projectionViewModel = rot2;

        FrameUniforms *uniforms4 = (FrameUniforms *) [_uniformBuffer4 contents];
        uniforms4->projectionViewModel = ed;
       ...
       // id <MTLRenderCommandEncoder> encoder3 = [commandBuffer renderCommandEncoderWithDescriptor:rpd2];

        [encoder2 setFrontFacingWinding:MTLWindingCounterClockwise];
        [encoder2 setRenderPipelineState:_pipelineState2];
        [encoder2 setVertexBuffer:_vertexBuffer2
                           offset:0
                          atIndex:MeshVertexBuffer];
        [encoder2 setVertexBuffer:_uniformBuffer3
                           offset:0 atIndex:FrameUniformBuffer];
        [encoder2 drawPrimitives:MTLPrimitiveTypeTriangleStrip vertexStart:0 vertexCount:4];
        [encoder2 endEncoding];

        id <MTLRenderCommandEncoder> encoder4 = [commandBuffer renderCommandEncoderWithDescriptor:rpd3];

        [encoder4 setRenderPipelineState:_pipelineState3];
        [encoder4 setVertexBuffer:_vertexBuffer3
                           offset:0
                         atIndex:MeshVertexBuffer];
        [encoder4 setVertexBuffer:_uniformBuffer4
                          offset:0 atIndex:FrameUniformBuffer];
        [encoder4 setFragmentTexture: _bufTxt atIndex: 0];

        [encoder4 drawPrimitives:MTLPrimitiveTypeTriangle vertexStart:0 vertexCount:2*3];
